<?php
/**
 * ACF Field Group: Child Theme: Introduction Block
 *
 * Description
 *   Creates a Gutenberg block intended for section introductions.
 *
 * Use
 *   This Singleton class only needs to be initializeed once by calling:
 *
 *      IntroductionBlock::init();
 *
...
class IntroductionBlock
{
    /**
     * Block name
     */
    private static $name = 'introduction-block';
        ...
        add_action('acf/init', function () {
            if ( function_exists( 'acf_register_block_type' )) {
                acf_register_block_type([
                    ...
                    'title'             => 'Introduction',
                    'description'       => 'Section to contain introducing page content.',
                    'render_callback'   => [$this, 'display_block'],
                    'category'          => 'child-theme',
                    'icon'              => "", // TODO
                    'keywords'          => ['child theme', 'colter', 'introduction', 'heading'],
    ...
    public function display_block( $block, $content = '', $is_preview = false, $post_id = 0 )
    {
         ...
         * Content
         */
        $heading       = is_string( get_field( 'heading' ) ) ? get_field( 'heading' ) : 'The Colter Apartments<br>Fort Collins';
        $heading_level = is_string( get_field( 'heading_level' ) ) ? get_field( 'heading_level' ) : 'h2';
        $line_position = is_numeric( get_field( 'line_position' ) ) ? get_field( 'line_position' ) : '0.65';
    ?>
        <style>
        /* Styles for our block below */

        #<?php echo $id; ?> .ct-heading > *:before {
                ...
                <div class="ct-container">
                    <?php if ( ! empty( $heading ) ) : ?>
                        <div class="ct-heading">
                            <<?php echo $heading_level; ?>><?php echo $heading; ?></<?php echo $heading_level; ?>>
                        </div>
                    <?php endif; ?>
